...
static void draw_edge_weight(int x, int y, int weight) {
    const int badge_r = 14;
    const char* text = TextFormat("%d", weight);
    int text_w = MeasureText(text, 18);

    DrawCircle(x, y, badge_r, (Color){20, 30, 65, 240});
    DrawCircleLines(x, y, (float)badge_r, (Color){255, 230, 140, 255});
    DrawText(text, x - text_w / 2, y - 9, 18, (Color){255, 248, 190, 255});
}
...
static void draw_ghost_node(Vector2 p, float size, Color body_color, int node_id) {
    ...
    int id_w = MeasureText(id_text, 16);
    ...
    DrawText(id_text, (int)(p.x - id_w / 2.0f), (int)(p.y + size * 0.15f), 16, (Color){15, 22, 45, 255});
}
...
static Point* build_layout(int n) {
    ...
    float r = (WINDOW_HEIGHT < WINDOW_WIDTH ? WINDOW_HEIGHT : WINDOW_WIDTH) * 0.34f;
...
static void render_scene(
    ...
    for (i = 0; i < graph->num_vertices; i++) {
        ...
        while (edge != NULL) {
            ...
            draw_edge_weight((int)((a.x + b.x) * 0.5f), (int)((a.y + b.y) * 0.5f + 16.0f), edge->weight);
            edge = edge->next;
        }
    }
    ...
    for (i = 0; i < graph->num_vertices; i++) {
        ...
        draw_ghost_node((Vector2){positions[i].x, positions[i].y}, (float)NODE_RADIUS * 1.2f, node_color, i);
    }
...
int main(void) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Week1 Program - Pacman Shortest Path (Raylib)");
    SetTargetFPS(60);
    last_time = GetTime();
