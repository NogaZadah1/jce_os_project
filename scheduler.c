...
int scheduler_enqueue_fcfs(
   ...
   request->traveler_id = traveler_id;
    ...
    request->remaining_cost = remaining_cost;
...
int scheduler_dequeue_sjf(
    ...
    SchedulerRequest* previous;
    SchedulerRequest* best;
    SchedulerRequest* best_previous;
    ...
    while (current != NULL) {
        if (current->remaining_cost < best->remaining_cost ||
            (current->remaining_cost == best->remaining_cost &&
             current->arrival_order < best->arrival_order)) {
            best = current;
